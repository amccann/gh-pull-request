...
export default class InputArea extends BaseComponent {
  ...
  constructor(content) {
    ...
    this.element = editModeHtmlNode(this.content, this.hasSaveButton);
    this.element.classList.add('inputArea');

    this.element.dom.saveButton.addEventListener('click', () => {
      this.save();
    });

    this.element.dom.textarea.addEventListener('keydown', (event) => {
